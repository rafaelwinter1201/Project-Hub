<div class="row pt-3">
    ...
    <input type="date" id="endDate" name="endDate" class="form-control w-10"
        value="{{ isset($response['filtros']['endDate']) ? $response['filtros']['endDate'] : '' }}"
        min="2000-12-01" max="{{ date('Y-m-d') }}" data-error="Data inválida."
        title="Data inválida." oninvalid="this.setCustomValidity('Data inválida.')"
        wire:model="endDate" @if($endDateDisabled) disabled @endif />
</div>
